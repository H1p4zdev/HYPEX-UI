...
# 1g. Gradle wrapper (gradlew, gradlew.bat, gradle-wrapper.jar)
# Generate using gradle wrapper if gradle is installed, otherwise skip
if command -v gradle &> /dev/null; then
    echo "  📦 Running 'gradle wrapper --gradle-version $GRADLE_VERSION'..."
    cd /tmp
    mkdir -p wrapper-init
    cd wrapper-init
    gradle wrapper --gradle-version "$GRADLE_VERSION" 2>/dev/null || true
    if [ -f gradlew ]; then
        cd "$OLDPWD"
        cp "/tmp/wrapper-init/gradlew" gradlew
        cp "/tmp/wrapper-init/gradlew.bat" gradlew.bat
        cp "/tmp/wrapper-init/gradle/wrapper/gradle-wrapper.jar" gradle/wrapper/gradle-wrapper.jar 2>/dev/null || true
    else
        cd "$OLDPWD"
    fi
else
    echo "  ⚠️  Gradle not found in PATH. Generate wrapper manually by running: gradle wrapper"
fi
